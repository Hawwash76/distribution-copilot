...
shadcn/ui, TanStack Query (server state), Zustand (UI state), and the typed tRPC client.
...
- Consume the API via the **typed tRPC client** with end-to-end type safety.

## Boundaries

**Depends on:** `@distribution-copilot/trpc` (**type only**), `shared`, `ui`, `config`.
...
│   └── hooks/        # use-opportunities.ts → useOpportunities (wraps tRPC)
├── components/       # cross-feature app components (providers.tsx, …)
├── lib/              # trpc.ts, query-client.ts, monitoring.ts, utils.ts
...
- **Data fetching = TanStack Query via tRPC.** `trpc.<feature>.<proc>.useQuery/useMutation`.
  Wrap feature access in a `use-*.ts` hook. Invalidate or optimistically update on mutation.
- **`AppRouter` is a type import** in `lib/trpc.ts` — zero runtime coupling to the API.
...
- **Map tRPC error codes to UX** (`UNAUTHORIZED` → sign-in, `TOO_MANY_REQUESTS` → backoff).
...
- New API data? Add the procedure in the API first (it's the contract); the typed hook
  follows automatically.
- Keep `lib/` for client wiring only (trpc, query client, monitoring, utils).
...
Key env: `NEXT_PUBLIC_TRPC_URL` (API tRPC endpoint), `NEXT_PUBLIC_POSTHOG_KEY/HOST`,
